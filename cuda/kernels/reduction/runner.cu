...
#include "04_reduce_idle_threads.cuh"
...
void run_reduction_reduce_idle_threads(float *d_v, float *d_v_r, int n) {
  int num_threads = REDUCTION_SEQUENTIAL_SIZE;
  int num_blocks = (n / 2 + num_threads - 1) / num_threads;
  reductionReduceIdleThreads<<<num_blocks, num_threads>>>(d_v, d_v_r);
}
